...
const ProductCardAccessories = ({item, name, price, image, hoverImage, discount, size, category, selectedGrid, index, pageType ,CallingFrom}) => {
    ...
    //           try {
    //               const response = await axios.get('http://localhost:5000/accessories');
    ...
    useEffect(() => {
        ...
        const fetchMattresses = async () => {
          try {
            const response = await axios.get("https://ottomonbeds-uk.vercel.app/accessories");
            console.log("API Response:___", response.data);
            setLoading(false)
            // setMattresses(response.data.accessoriesData);
             const filters = {
              Pillows: 'Pillows',
              Duvetts: 'Duvetts',
            ...
            console.log(mattresses," mattresses accessories data ")
          } catch (error) {
            console.error("Error fetching data:", error);
            setLoading(false)
          }
              ...
              <div key={item._id} className="min-w-full min-h-full bg-white shadow-md rounded-xl  hover:shadow-xl overflow-hidden">
                <div className="overflow-hidden relative">
                  ...
                  <img
                    src={item.imageUrl}
                    alt="Product"
                    className=" h-[300px] w-[100%] rounded-t-xl duration-1000 hover:scale-125"
                    ref={imageRef}
                  />
                </div>
                <div className="text-center pt-3">
